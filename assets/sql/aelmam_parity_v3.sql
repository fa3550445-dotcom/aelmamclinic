...
-- Works on SQLite 3.10 (no window functions).
-- =========================================================
...
-- ---------------------------------------------------------
-- 1) إضافة أعمدة المزامنة لكل الجداول (مرة واحدة)
--    SQLite 3.10 لا يدعم IF NOT EXISTS للعمود؛
--    لذلك لو العمود موجود ستظهر رسالة خطأ ويتم المتابعة (بفضل .bail off).
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 2) تعبئة أعمدة المزامنة الفارغة بقيم صحيحة
--    account_id/device_id من sync_identity
--    local_id من rowid لأول مرة
--    updated_at الآن إن كانت NULL
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 3) إزالة التكرارات (acc_id,dev_id,local_id) بدون WINDOW
--    نبقي الأحدث حسب updated_at ثم الأكبر rowid.
--    (بدون "DELETE ... AS" لضمان التوافق مع 3.10)
-- ---------------------------------------------------------
...
-- ---------------------------------------------------------
-- 4) إزالة تكرارات الأسماء الطبيعية لتطابق فهارس السحابة
--    drugs: (account_id, lower(name))
--    items: (account_id, type_id, name)
-- ---------------------------------------------------------
DELETE FROM drugs
WHERE EXISTS (
  SELECT 1 FROM drugs AS b
...
-- ---------------------------------------------------------
-- 6) Triggers لتحديث updated_at مرة واحدة بعد UPDATE
--    (AFTER UPDATE + شرط يمنع الحلقة اللانهائية)
-- ---------------------------------------------------------
