...
class RadiologyServicesScreen extends StatefulWidget {
  ...
  @override
  State<RadiologyServicesScreen> createState() =>
      _RadiologyServicesScreenState();
}

class _RadiologyServicesScreenState extends State<RadiologyServicesScreen> {
  ...
  List<Map<String, dynamic>> _radiologyServices = [];
  List<Map<String, dynamic>> _filtered = [];

  @override
  void initState() {
    super.initState();
    _loadRadiologyServices();
  ...
  /*────────────────── تحميل / فلترة ──────────────────*/
  Future<void> _loadRadiologyServices() async {
    setState(() {
      _busy = true;
      _error = null;
    });
    try {
      ...
      setState(() {
        _radiologyServices = services;
        _filtered = List<Map<String, dynamic>>.from(services);
      });
    } catch (e) {
      setState(() => _error = 'فشل تحميل خدمات الأشعة: $e');
    } finally {
  ...
  void _applyFilter() {
    ...
    setState(() {
      if (q.isEmpty) {
        _filtered = List<Map<String, dynamic>>.from(_radiologyServices);
      } else {
        _filtered = _radiologyServices.where((e) {
  ...
  /*────────────────── إضافة/تعديل خدمة ──────────────────*/
  Future<void> _addOrEditRadiologyService({int? serviceId}) async {
    final isEdit = serviceId != null;
    final title = isEdit ? 'تعديل خدمة الأشعة' : 'إضافة خدمة أشعة';

    if (isEdit) {
      final old = _radiologyServices.firstWhere((e) => e['id'] == serviceId,
          orElse: () => {});
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                final cost =
                    _parseCost(_serviceCostCtrl.text /* يدعم أرقام عربية */) ??
                        0;
  ...
  /*────────────────── حذف ──────────────────*/
  Future<void> _deleteRadiologyService(int serviceId) async {
  ...
  Future<void> _importRadiologyServicesFromExcel() async {
    ...
    try {
      // خريطة بالإسم الموحّد → السجل الحالي (للتحديث بدل التكرار)
      final nameToRow = <String, Map<String, dynamic>>{};
      for (final e in _radiologyServices) {
      ...
      for (final sheetName in excel.tables.keys) {
        ...
        // تخطّي الصف الأول كعناوين: [نوع الخدمة, الكلفة]
  ...
  Future<void> _downloadExcelTemplate() async {
    try {
      ...
      // أمثلة
      sheet.appendRow(['أشعة سينية للصدر', '3000']);
      sheet.appendRow(['ألتراساوند (بطن/حوض)', '5000']);
      sheet.appendRow(['CT Scan (دماغ)', '12000']);

      final bytes = excel.encode()!;
      final dir = await getTemporaryDirectory();
      final file = File('${dir.path}/نموذج_خدمات_أشعة.xlsx');
  ...
  List<Widget> _buildBody(ColorScheme scheme) {
    if (_error != null) {
      return [
        _ErrorCard(message: _error!, onRetry: _loadRadiologyServices),
      ];
    }
    if (_busy && _radiologyServices.isEmpty) {
      return const [
        SizedBox(height: 100),
                    ...
                    children: [
                      Expanded(
                        child: TOutlinedButton(
                          ...
                          label: 'تعديل',
                          onPressed: () =>
                              _addOrEditRadiologyService(serviceId: id),
                        ),
                      ),
                      const SizedBox(width: 10),
                      NeuButton.flat(
                        ...
                        label: 'حذف',
                        onPressed: () => _deleteRadiologyService(id),
                      ),
                    ],
                  ),
                ],
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: RefreshIndicator(
            ...
            onRefresh: _loadRadiologyServices,
            child: ListView(
